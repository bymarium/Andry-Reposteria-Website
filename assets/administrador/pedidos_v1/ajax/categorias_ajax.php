<?php
	...
	$query_validate=mysqli_query($con,"select category_id from products where category_id='".$id."'");
	$count=mysqli_num_rows($query_validate);
	if ($count==0){
			if($delete=mysqli_query($con, "DELETE FROM categories WHERE id='$id'")){
				$aviso="Bien hecho!";
				...
				<div class="box-body">
					...
					<table class="table table-condensed table-hover table-striped">
						...
						while($row = mysqli_fetch_array($query)){	
							...
							$count=mysqli_query($con,"select count(*) AS num_prod from products where category_id='".$id."'" );
