			...
			<div class="table-responsive">
			  <table class="table">
				...
				while ($row=mysqli_fetch_array($query)){
					...
					$buying_price=number_format($buying_price,2,'.','');
					?>
					<tr>
						...
						<td class='col-xs-2'>
							<select class="form-control" required id="id_sucursal_<?php echo $id_producto; ?>">
								<option value="">Selecciona</option>
						<?php
							$list_branch_offices=list_branch_offices();
							while ($list=mysqli_fetch_array($list_branch_offices)){
								$id_sucursal=$list['id'];
								$nombre_sucursal=$list["name"];
								?>
